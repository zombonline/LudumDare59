...
#if EPO_HDRP
float _Side;
UNITY_DECLARE_TEX2DARRAY(_DepthTarget);
float _DepthClipTest;
#endif

#if EPO_HDRP
float SampleDepth(float2 uv)
{
	return LinearEyeDepth(UNITY_SAMPLE_TEX2DARRAY(_DepthTarget, float3(uv * _Scale, 0.0f)).r);	
}

void ClipByDepth(float2 uv, float currentDepth)
{
	if (_Side == 0.0f)
		return;
	
	if (SampleDepth(uv) * _Side <= currentDepth * 0.95f * _Side)
		discard;
}

#define CLIP_ON_DEPTH(uv,currentDepth) ClipByDepth(uv,currentDepth)
#else
#define CLIP_ON_DEPTH(uv,currentDepth)
#endif
...
#define FetchTexelAt(uv) FetchTexelAtFrom(_MainTex,(uv),_MainTex_ST)
#define FetchTexelAtWithShift(uv,shift) UNITY_SAMPLE_SCREENSPACE_TEXTURE(_MainTex,UnityStereoScreenSpaceUVAdjust((uv)+(shift),(_MainTex_ST)))
...
#define SHIFT_AMOUNT 0.0001f
#if defined(UNITY_REVERSED_Z) 
#define ChangeDepth o.vertex.z += SHIFT_AMOUNT;
#else
#define ChangeDepth o.vertex.z -= SHIFT_AMOUNT;
#endif
