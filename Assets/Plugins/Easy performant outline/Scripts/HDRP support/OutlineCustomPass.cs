...
namespace EPOOutline
{
    ...
    public class OutlineCustomPass : CustomPass
    {
        ...
        private void UpdateOutliner(ScriptableRenderContext renderContext, CommandBuffer cmd, Camera camera, Outliner outlineEffect, HDCamera hdCamera, RTHandle colorTarget, RTHandle depthTarget)
        {
            ...
            parameters.TargetWidth = camera.pixelWidth;
            parameters.TargetHeight = camera.pixelHeight;
            parameters.Antialiasing = colorTarget.rt.antiAliasing;

            var viewportSize = new Vector2Int(parameters.TargetWidth, parameters.TargetHeight);
            parameters.Viewport = new Rect(Vector2.zero, viewportSize);
            
            var targetViewportSize = colorTarget.useScaling
            ...
            parameters.Scale =  targetViewportSize / new Vector2(colorTarget.rtHandleProperties.currentRenderTargetSize.x, colorTarget.rtHandleProperties.currentRenderTargetSize.y);
            
            parameters.TargetViewport = new Rect(Vector2.zero, targetViewportSize);
            ...
            parameters.FinalBlitApproach = FinalBlitApproach.IntermediateTexture;
