...
namespace EPOOutline
{
    public static class OutlineEffect
    {
        ...
        public static readonly int DepthTarget = Shader.PropertyToID("_DepthTarget");
        public static readonly int SideHash = Shader.PropertyToID("_Side");
        ...
        private static Material FinalFullScreenBlitMaterial;
        ...
        private static void Postprocess(OutlineParameters parameters, RTHandle first, RTHandle second, Material material, int iterations, int eyeSlice, bool additionalShift, float shiftValue, ref int stencil, float scale)
        {
            if (iterations <= 0)
                return;
            ...
            for (var index = 1; index <= iterations; index++)
            {
                ...
                Blit(parameters, first, second, first, material, shiftValue, eyeSlice, -1);

                stencil = (stencil + 1) % 255;
                parameters.Buffer.SetGlobalInt(RefHash, stencil);

                parameters.Buffer.SetGlobalVector(ShiftHash, new Vector4(0, shift * scale));
                Blit(parameters, second, first, first, material, shiftValue, eyeSlice, -1);
        ...
        public static void SetupOutline(OutlineParameters parameters)
        {
            ...
            if (parameters.DepthTestKind == DepthTestKind.DepthTexture)
                parameters.Buffer.SetGlobalTexture(DepthTarget, parameters.DepthTarget);
            ...
            if (parameters.FinalBlitApproach == FinalBlitApproach.IntermediateTexture)
            {
                parameters.Buffer.SetRenderTarget(parameters.Handles.FinalizationTarget, -1);
                parameters.Buffer.ClearRenderTarget(true, true, Color.clear);
            }
            ...
            parameters.Buffer.SetRenderTarget(parameters.Handles.Target,
                parameters.DepthTestKind == DepthTestKind.DepthTexture
                    ? parameters.Handles.Target
                    : parameters.DepthTarget, eyeDepthSlice);
            
            parameters.Buffer.ClearRenderTarget(false, true, Color.clear);
            
            DrawOutlineables(parameters,
                1.0f,
                CompareFunction.LessEqual,
                x => true,
                x => Color.clear,
                x => ZPrepassMaterial,
                RenderStyle.FrontBack | RenderStyle.Single,
                OutlinableDrawingMode.ZOnly);
            ...
            if (parameters.UseInfoBuffer)
            {
                ...
                parameters.Buffer.SetRenderTarget(parameters.Handles.InfoTarget,
                    parameters.DepthTestKind == DepthTestKind.DepthTexture
                        ? parameters.Handles.Target
                        : parameters.DepthTarget, eyeDepthSlice);
                ...
                DrawOutlineables(parameters, 
                    ...
                    x => x.OutlineParameters.Enabled,
                    x => new Color(x.OutlineParameters.DilateShift, x.OutlineParameters.BlurShift, 0, 1),
                    x => OutlineMaterial,
                    RenderStyle.Single, 
                ...
                DrawOutlineables(parameters,
                    ...
                    x => x.BackParameters.Enabled,
                    x => new Color(x.BackParameters.DilateShift, x.BackParameters.BlurShift, 0, 1),
                    x => OutlineMaterial,
                ...
                DrawOutlineables(parameters,
                    1.0f,
                    CompareFunction.LessEqual,
                    x => x.FrontParameters.Enabled,
                    x => new Color(x.FrontParameters.DilateShift, x.FrontParameters.BlurShift, 0, 1),
                    x => OutlineMaterial,
                    RenderStyle.FrontBack, 
                    OutlinableDrawingMode.Normal);
                ...
                DrawOutlineables(parameters,
                    ...
                    CompareFunction.LessEqual,
                    x => true,
                    x => new Color(0, 0, GetMaskingValueForMode(x.DrawingMode), 1),
                    x => ObstacleMaterial,
            ...
            drawnOutlinablesCount += DrawOutlineables(parameters,
                0.0f,
                CompareFunction.Always,
                x => x.OutlineParameters.Enabled,
                x => x.OutlineParameters.Color,
                x => OutlineMaterial,
            ...
            if (parameters.FinalBlitApproach == FinalBlitApproach.IntermediateTexture && 
                parameters.Buffer is IBlitCapableCommandBufferWrapper blitCapableCommandBufferWrapper)
            {
                if (FinalFullScreenBlitMaterial == null)
                    FinalFullScreenBlitMaterial = LoadMaterial("FinalFullScreenBlit");
                
                blitCapableCommandBufferWrapper.Blit(parameters.Handles.PrimaryTarget, parameters.Handles.FinalizationTarget, FinalFullScreenBlitMaterial, 0);
                DrawOutlineables(
                    parameters,
                    0.0f,
                    CompareFunction.Always,
                ...
                DrawFill(parameters, parameters.Handles.FinalizationTarget, parameters.Handles.FinalizationStencilTarget);
                
                blitCapableCommandBufferWrapper.Blit(parameters.Handles.FinalizationTarget, parameters.Target, FinalFullScreenBlitMaterial, 0); 
            }
            else
            {
                Blit(parameters,
                    ...
                    parameters.TargetViewport);
                
                DrawFill(parameters, parameters.Target, parameters.DepthTestKind == DepthTestKind.DepthTexture ? parameters.Target : parameters.DepthTarget);
        ...
        private static int DrawOutlineables(OutlineParameters parameters, float side, CompareFunction function, Func<Outlinable, bool> shouldRender, Func<Outlinable, Color> colorProvider, Func<Outlinable, Material> materialProvider, RenderStyle styleMask, OutlinableDrawingMode modeMask = OutlinableDrawingMode.Normal)
        {
            var drawnCount = 0;
            parameters.Buffer.SetGlobalFloat(SideHash, side);
            parameters.Buffer.SetGlobalInt(ZTestHash, parameters.DepthTestKind == DepthTestKind.DepthTexture ? (int)CompareFunction.Always : (int)function);
        ...
        private static void DrawFill(OutlineParameters parameters, RTHandle targetSurface, RTHandle depthTargetSurface)
        {
            var eye = RenderTargetUtility.GetDepthSliceForEye(parameters.EyeMask);
            parameters.Buffer.SetRenderTarget(targetSurface, depthTargetSurface, eye);
            
            if (parameters.TargetViewport.HasValue)
                parameters.Buffer.SetViewport(parameters.TargetViewport.Value);

            var singleMask = 1;
            var frontMask = 2;
            var backMask = 3;

            parameters.Buffer.SetGlobalInt(ZTestHash, parameters.DepthTestKind == DepthTestKind.DepthTexture ? (int)CompareFunction.Always : (int)CompareFunction.Greater);
            parameters.Buffer.SetGlobalFloat(SideHash, -1);
            ...
            parameters.Buffer.SetGlobalInt(ZTestHash, parameters.DepthTestKind == DepthTestKind.DepthTexture ? (int)CompareFunction.Always : (int)CompareFunction.LessEqual);
            parameters.Buffer.SetGlobalFloat(SideHash, 1);
            ...
            foreach (var outlinable in parameters.OutlinablesToRender)
            {
                ...
                else
                {
                    ...
                    parameters.Buffer.SetGlobalFloat(SideHash, 0);
        ...
        private static void SetupCutout(OutlineParameters parameters, OutlineTarget target)
        {
            ...
            if (target.Renderer is SpriteRenderer spriteRenderer)
            {
                ...
                parameters.Buffer.SetGlobalTexture(CutoutTextureHash, parameters.TextureHandleMap[spriteRenderer.sprite.texture]);
