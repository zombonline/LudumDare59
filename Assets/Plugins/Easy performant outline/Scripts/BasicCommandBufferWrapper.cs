...
{
    public class BasicCommandBufferWrapper : CommandBufferWrapper, IUnderlyingBufferProvider, IDisposable, IBlitCapableCommandBufferWrapper
    {
        ...
        public void Blit(RTHandle source, RTHandle destination, Material material, int pass)
        {
            var propertyBlock = new MaterialPropertyBlock();
            propertyBlock.SetTexture(InputTexture, source);
            CoreUtils.SetRenderTarget(buffer, destination);
            buffer.DrawProcedural(Matrix4x4.identity, material, pass, MeshTopology.Triangles, 3, 1, propertyBlock);
        }
